...
if [ ${JAVA_MAJOR_VERSION:-0} -ge @javaVersion@ ]; then
  ...
  echo "Exiting: Defendly requires a minimum of Java @javaVersion@ to run, found $JAVA_VERSION"
  exit 1
fi
...
if [ "$OS" = "Darwin" ]; then
  ...
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -Xdock:icon="../Resources/Defendly.icns" -jar "${BASEDIR}/@zapJar@" "${ARGS[@]}"
else
