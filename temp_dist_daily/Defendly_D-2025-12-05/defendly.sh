...
# Base directory where Defendly is installed
BASEDIR="`pwd -P`"

# Switch to the directory where Defendly is installed
cd "$BASEDIR"
...
if [ "`echo ${JAVA_OUTPUT} | grep "continuing with system-provided Java"`" ] ; then
  ...
  echo "    " $JAVA_OUTPUT
  echo "Unsetting JAVA_HOME and continuing with Defendly start-up"
  unset JAVA_HOME
fi
...
if [ ${JAVA_MAJOR_VERSION:-0} -ge 17 ]; then
  ...
  echo "Exiting: Defendly requires a minimum of Java 17 to run, found $JAVA_VERSION"
  exit 1
fi

if [ "$OS" = "Darwin" ]; then
  JVMPROPS="$HOME/Library/Application Support/Defendly/.Defendly_JVM.properties"
else
  JVMPROPS="$HOME/.Defendly/.Defendly_JVM.properties"
fi
...
# Start Defendly; it's likely that -Xdock:icon would be ignored on other platforms, but this is known to work
if [ "$OS" = "Darwin" ]; then
  # It's likely that -Xdock:icon would be ignored on other platforms, but this is known to work
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -Xdock:icon="../Resources/Defendly.icns" -jar "${BASEDIR}/zap-D-2025-12-05.jar" "${ARGS[@]}"
else
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -jar "${BASEDIR}/zap-D-2025-12-05.jar" "${ARGS[@]}"
fi
